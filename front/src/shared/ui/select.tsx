...
export const Select = ({
  ...
  const useTriggerVariant = composeVariants({
    beforeClasses: makeUiBaseClasses(makeUiClickableClasses()),
    afterClasses: makeClasses(className, triggerClasses),
    outlined,
    defaultVariants: {
      scheme,
      padding: size,
  ...
  const contentClasses = makeClasses("py-2", "bg-background-soft", "fill-background-soft", "rounded-xl", "shadow-xs")
  ...
  return (
    <RadixSelect.Root value={value} defaultValue={defaultValue} onValueChange={onChangeValue}>
      <RadixSelect.Trigger className={useTriggerVariant({})}>
